...
import {
  TrendingUp, TrendingDown, Wallet, PieChart, PlusCircle, Calendar,
  CheckSquare, ArrowUpRight, X, LogOut
} from 'lucide-react';
...
useEffect(() => {
  ...
  if (loading) {
    return <div className="flex items-center justify-center min-h-screen bg-slate-950 text-white">Loading Dashboard...</div>
  }
    ...
    <div className="space-y-8 p-6 text-slate-100 font-sans relative bg-slate-950 min-h-screen">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        <div className="p-5 rounded-2xl bg-slate-900/60 border-slate-800/80 backdrop-blur-xl shadow-xl hover:border-slate-700/60 transition-all">
          <div className="flex justify-between items-center text-slate-400 mb-2"><span className="text-sm font-medium">Total Saldo</span><Wallet className="w-5 h-5 text-cyan-400" /></div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2 p-6 rounded-2xl bg-slate-900/60 border-slate-800/80 backdrop-blur-xl shadow-xl">
          <h2 className="text-lg font-bold text-white mb-4 flex items-center gap-2"><PieChart className="w-5 h-5 text-cyan-400" /> Batas & Progress Budget</h2>
        ...
        <div className="p-6 rounded-2xl bg-slate-900/60 border border-slate-800/80 backdrop-blur-xl shadow-xl space-y-4">
          ...
          <div className="space-y-3">
            <div className="p-3 rounded-xl bg-slate-800/40 border-slate-700/40 flex items-start gap-3"><div className="w-2 h-2 rounded-full bg-cyan-400 mt-2" /><div><p className="text-sm font-semibold text-slate-200">Rapat Panitia Seminar</p><p className="text-xs text-slate-400">19:30 WIB • Ruang Utama Org</p></div></div>
      ...
      {activeModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-sm p-4">
          <div className="bg-slate-900 border border-slate-800 rounded-2xl p-6 w-full max-w-md shadow-2xl relative">
            <button onClick={() => setActiveModal(null)} className="absolute top-4 right-4 text-slate-400 hover:text-white transition-colors cursor-pointer"><X className="w-5 h-5" /></button>
            <h3 className="text-xl font-bold text-white capitalize mb-4">Tambah {activeModal}</h3>
            <form onSubmit={(e) => { e.preventDefault(); alert(`Data ${activeModal} berhasil disimpan!`); setActiveModal(null); }} className="space-y-4">
              {/* isi form kamu */}
            </form>
          </div>
        </div>
