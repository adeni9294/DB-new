...
export async function createTransaction(payload: TransactionDTO) {
  try {
    ...
    // 1. Dapatkan USER_ID yang valid dari database (bukan string hardcode)
    ...
    // 2. Dapatkan ACCOUNT_ID yang valid dari database
    ...
    if (!validAccountId || validAccountId === 'DEFAULT_ACCOUNT' || validAccountId === 'ACC-DEFAULT') {
      ...
      if (accRes?.rows?.[0]) {
        ...
        // Buat akun baru dengan ID unik jika tabel accounts masih kosong
    ...
    // 3. Simpan transaksi dengan data yang dipastikan terisi
    const sql = `
